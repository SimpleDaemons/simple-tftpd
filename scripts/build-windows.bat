...
:build_project
call :print_status "Building simple-tftpd..."
...
:install_project
if "%INSTALL%"=="true" (
    call :print_status "Installing simple-tftpd..."
    cd /d "%BUILD_DIR%"
...
if "%ENABLE_STATIC_LINKING%"=="ON" (
    ...
    set "STATIC_DIR=%DIST_DIR%\simple-tftpd-0.1.0-static-windows"
    if not exist "%STATIC_DIR%" mkdir "%STATIC_DIR%"
    
    REM Copy binary and files
    copy "%BUILD_DIR%\simple-tftpd.exe" "%STATIC_DIR%\"
    ...
    cd /d "%DIST_DIR%"
    powershell -Command "Compress-Archive -Path 'simple-tftpd-0.1.0-static-windows' -DestinationPath 'simple-tftpd-0.1.0-static-windows.zip' -Force"
    rmdir /s /q "simple-tftpd-0.1.0-static-windows"
    
    call :print_success "Static binary package created: simple-tftpd-0.1.0-static-windows.zip"
)
goto :eof

REM Function to show usage
:show_usage
echo simple-tftpd - Windows Build Script
echo.
...
REM Main execution
:main
call :print_status "Starting simple-tftpd build process..."
...
call :print_success "simple-tftpd build process completed successfully!"
goto :end
