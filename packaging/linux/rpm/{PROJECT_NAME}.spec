...
Version:        ${VERSION}
Release:        1%{?dist}
Summary:        simple-tftpd - A lightweight and secure daemon
License:        Apache-2.0
...
%description
simple-tftpd is a modern, secure implementation of the TFTP protocol.
...
if [ "$1" -eq 1 ]; then
    ...
    # Create service user
    if ! id tftpddev &>/dev/null; then
        useradd -r -s /sbin/nologin -d /var/lib/%{name} -c "%{name} service user" tftpddev
    fi
fi
