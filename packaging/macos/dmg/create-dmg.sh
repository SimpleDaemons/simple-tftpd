#!/bin/bash
# Create DMG installer for simple-tftpd
...
VERSION="${1:-1.0.0}"
DMG_NAME="$simple-tftpd-${VERSION}"
DMG_TEMP="dmg-temp"
DMG_FILE="${DMG_NAME}.dmg"
APP_NAME="$simple-tftpd.app"
PKG_NAME="$simple-tftpd.pkg"
...
if [ -f "../assets/icons/$simple-tftpd.icns" ]; then
    # Set custom icon (requires additional tools)
    echo "Custom icon available at ../assets/icons/$simple-tftpd.icns"
fi
